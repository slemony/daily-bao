...
- **Per-source translation mode**: each feed has a `translate` field — `'off' | 'auto' | 'manual'` (default `'off'`). Set in Add/Edit feed modals; the field is hidden when `preferredLang` is not set. Both modes are skipped silently when `FEED_LANG_TO_BCP47[article.lang] === preferredLang` (article already in target language)
- **Auto-translate** (`feed.translate === 'auto'`): after `cleanReaderContent()` renders prose into `#reader-content`, a `.trans-overlay` loading indicator appears while `translateProse()` runs. All `p`, `h2`, `h3`, `h4`, `li`, `blockquote`, `div` (leaf only) elements are collected, batched into ≤4500-char chunks joined with `TRANS_SEP` (`\u0000||||\u0000`), sent to the translation API, then split back and applied. On delimiter mismatch, falls back to element-by-element. Aborted via `readSession._transAbort` (AbortController) when reader closes. Double-click still works in auto mode but without hover indicators
- **Manual translate** (`feed.translate === 'manual'`): `attachManualTranslateListeners(prose)` adds `trans-enabled` class to `.reader-prose` and wires up interactions. **Double-click** on desktop; **long-press (600ms)** on mobile/touch. Triggers `openInlineTranslation(el)`
- **Inline translation block** (`.trans-inline`): inserted immediately after the target paragraph via `el.after(div)`. Slides open with `max-height` CSS transition. Shows language label + ✕ close button. Double-clicking the same paragraph again toggles it closed. Clicking ✕ or `closeInlineTranslation()` collapses with transition then removes from DOM. The block is fully excluded from translation targeting — `pickEl` checks `target.closest('.trans-inline')` and CSS resets cursor/outline inside it
- **`pickEl(target)`** in `attachManualTranslateListeners`: walks up through inline elements (`span`, `a`, `em`, `strong`, etc.) using a `BLOCK_TAGS` Set to find the nearest block ancestor inside `.reader-prose`. Skips container `<div>` elements that wrap other block children. Guards against clicking inside `.trans-inline`
- **Hover indicator** (`trans-enabled`): dashed accent outline + subtle background tint on hover — only added in `manual` mode, not `auto`. Signals "double-click / long-press to translate" without cluttering auto-translated articles
- **Long-press sweep animation** (`.trans-pressing`): added to the element on press start, removed on cancel or fire. A 2px accent line animates left→right across the bottom over 600ms (`@keyframes trans-press-sweep`) as a progress indicator
- **Translation API**: Google Translate unofficial (`translate.googleapis.com/translate_a/single?client=gtx`) as primary (best CJK quality, no API key). Falls back to MyMemory (`api.mymemory.translated.net`) on failure. Both calls respect the `AbortController` signal
- **`applyTranslation(el, text)`**: replaces `textContent` for plain elements; walks text nodes only when element contains `<a>`, `<strong>`, `<em>` etc. to preserve inline markup
- **Post-translation guards**: adds `is-cjk` class to `.reader-prose` when target is a CJK language; adds `dir="rtl"` for Arabic/Hebrew/Farsi/Urdu
- **`TRANS_LANG_NAMES`**, `TRANS_RTL_LANGS`, `TRANS_CJK_LANGS`, `FEED_LANG_TO_BCP47` — lookup maps/sets defined as constants in `app.js`
